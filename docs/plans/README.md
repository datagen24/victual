...
| 03 | [Category minimum stock](03-category-min-stock.md) | Landed | Wave 3b; PostgreSQL; migration 0268. Note-only shopping list row (Q1) remains a follow-up. **The nested `parent_product_group_id` follow-on is now [30](30-nested-product-groups.md)'s**, after 07-Q6 landed on taxonomy; whether a parent group's minimum rolls up to descendant groups is that plan's Q1 and touches `product_groups_missing`, which this plan shipped. |
...
| 07 | [Nested products](07-nested-products.md) | **Retired 2026-09-13** | Q6 answered **taxonomy** from a deliberate sampling of the real catalogue — seven of thirteen candidate pairs are separate products and none is a pool, so the roll-up relation had nothing to act on. [ADR-0023](../adr/0023-taxonomy-is-groups-packaging-is-parent-product.md) records it; [30](30-nested-product-groups.md) and [31](31-directed-substitution.md) own the work. The body stays as the research that made the question answerable — its eight-site audit is the cost figure that retired it. Nothing in it is scheduled. |
...
| 27 | [Label templates and rendering](27-label-templates-and-rendering.md) | Implemented except the designer | Wave 3b, alongside 25. Owns the browser designer, the headless renderer, previews and print artifacts; 25 keeps identity, jobs, printer configuration and the delivery worker. Gated on [ADR-0021](../adr/0021-label-templates-are-application-data.md), **accepted 2026-09-07** with all six prerequisites met — **gate cleared**. **No tracking issue yet**, unlike 25's #93, and 06 depends on this plan as well as on 25: a print job is not claimable until a validated artifact is attached. Migrations inventoried, not reserved. Owns sweep finding S32. |
| 28 | [Open container measurement](28-open-container-measurement.md) | Draft | Wave 4, alongside [08](08-nested-locations.md)'s successor work. Owns [ADR-0022](../adr/0022-open-containers-carry-a-measured-remainder.md), **Proposed** — an opened unit's remaining contents live on the `stock` entry, so counting pieces stops costing the ability to know a jug is half empty. Shares `stock_current`'s aggregation rewrite with [07](07-nested-products.md), which is why the two are scheduled together rather than in sequence. Additive on the stock reads, so it wants to land before [14](14-contract-and-regression-scaffolding.md) piece 2 freezes the contract. Migration 0277 reserved. |
| 29 | [Working container replenishment](29-working-container-replenishment.md) | Draft | Wave 4. Bagged backstock feeds a kitchen bin: pack sizes as barcodes on one product, the bin as a location, refill as a `TransferProduct` — all of which already exist. Adds a **(product, location) minimum** whose consequence is a refill prompt, never a shopping list entry, and consumes [ADR-0022](../adr/0022-open-containers-carry-a-measured-remainder.md) decision 4's entry-scoped tare to weigh the bin. Product-scoped tare cannot do this: `TransferProduct()` refuses tare-enabled products outright. Migration 0278 reserved. |
| 30 | [Nested product groups](30-nested-product-groups.md) | Draft | Wave 4. The taxonomy 07-Q6 chose: `product_groups.parent_product_group_id`, `product_groups_resolved`, and `UNIQUE(parent_product_group_id, name) NULLS NOT DISTINCT` replacing the global name constraint. Copies [08](08-nested-locations.md) almost exactly and is the **second consumer of `hierarchy_depth_limit()`** — not the one it was written generic for. Owns [ADR-0023](../adr/0023-taxonomy-is-groups-packaging-is-parent-product.md)'s decision 1 and the follow-on [03](03-category-min-stock.md) reserved. Migration 0279 reserved. |
| 31 | [Directed substitution](31-directed-substitution.md) | Draft | Wave 4. Beans substitute for grounds and never the reverse. Needed because 07-Q6 made those forms **separate products**, and today substitution is only a view over parentage — with no shared parent there is no relation at all. Owns ADR-0023 decision 4. Additive on the reads, so it wants to land before [14](14-contract-and-regression-scaffolding.md) piece 2 freezes the contract. Migration 0280 reserved. |
...
| 4 | 08 nested locations, the hierarchy selected by 07-Q6 — **which chose groups, so 30 and 31 rather than 07** — with 28 and 29 beside them | **08 complete, 2026-09-09**: migration 0273, `locations_resolved`, the depth function 07 will share, and the paths, parent picker and rolled-up filters in the UI. The product half is unchanged — taxonomy means an additive parent-group change after 03; packaging means 07 after 08 has been used. Resolve Q6 before scheduling the product work. **[28](28-open-container-measurement.md) joined the wave 2026-09-09** and does not wait on Q6: it edits `stock_current` below the product, where 07 edits it above, and running them apart means rewriting that aggregation twice. **[29](29-working-container-replenishment.md) joined the same day** and waits on nothing but ADR-0022's entry-scoped tare — most of what it needs is already shipped. **Q6 was answered 2026-09-13 and chose taxonomy**, so the product half of this wave is [30](30-nested-product-groups.md) and [31](31-directed-substitution.md), not [07](07-nested-products.md), which is retired. 30 is small — it is [08](08-nested-locations.md) with the nouns changed — and 31 exists because making the forms separate products removed the only relation the schema could express between them. |
