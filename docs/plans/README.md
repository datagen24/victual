...
| 30 | [Nested product groups](30-nested-product-groups.md) | Draft, **scheduled 2026-09-14, ready — [issue 148](https://github.com/datagen24/victual/issues/148) fixed**, [issue 124](https://github.com/datagen24/victual/issues/124) | Wave 4, first of the two product plans; ADR-0023 accepted 2026-09-14 and 07 retired the same day. The nesting trigger this plan copies fired only on `UPDATE` and checked only one of the two directions a violation can arrive from — both fixed by 148, migration `0277.pgsql.sql`; copy the trigger as that migration leaves it. The taxonomy 07-Q6 chose: `product_groups.parent_product_group_id`, `product_groups_resolved`, and `UNIQUE(parent_product_group_id, name) NULLS NOT DISTINCT` replacing the global name constraint. Copies [08](08-nested-locations.md) almost exactly and is the **second consumer of `hierarchy_depth_limit()`** — not the one it was written generic for. Owns [ADR-0023](../adr/0023-taxonomy-is-groups-packaging-is-parent-product.md)'s decision 1 and the follow-on [03](03-category-min-stock.md) reserved. Migration 0278 reserved, renumbered from 0277 to make room for 148's own 0277. |
