...
| 27 | [Label templates and rendering](27-label-templates-and-rendering.md) | Implemented; one follow-up | Wave 3b, alongside 25. Owns the browser designer, the headless renderer, previews and print artifacts; 25 keeps identity, jobs, printer configuration and the delivery worker. Gated on [ADR-0021](../adr/0021-label-templates-are-application-data.md), accepted 2026-09-07 — **gate cleared**. Migrations 0271 and 0272 are in `master`; the designer landed in PR 113 (2026-09-09), and verification 4's physical print and scan-back were met the same day. The designer's fabric 5.x migration, which could not be a dependency bump, **landed 2026-09-15** ([issue 126](https://github.com/datagen24/victual/issues/126)) — see its Executed section for the origin-default regression the move found and fixed, which a dependency bump alone would have shipped invisibly. Remaining: sweep finding S32, the fail-closed group-to-read-permission table for the files API ([issue 136](https://github.com/datagen24/victual/issues/136)). Question 9, where red belongs, waits on a `stock_entry` field kind behind ADR-0019 item 7 step 2. |
