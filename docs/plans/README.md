...
| 07 | [Nested products](07-nested-products.md) | Blocked on Q6 — **answered, not yet ratified** | The sampling Q6 asked for was done 2026-09-13 and landed on **taxonomy**: seven of thirteen candidate pairs are separate products and none is a pool, so the roll-up relation had nothing to act on. That is evidence, not authority — [ADR-0023](../adr/0023-taxonomy-is-groups-packaging-is-parent-product.md) is **Proposed** with four prerequisites outstanding, and this plan is retired **after** that record is accepted, by a separate pull request — the acceptance itself carries bookkeeping only. Until that later PR lands, this stays blocked and [30](30-nested-product-groups.md)/[31](31-directed-substitution.md) are not its replacements. |
...
| 4 | 08 nested locations; the hierarchy selected by 07-Q6, **pending ADR-0023's acceptance**; 28 and 29 beside them | **08 complete, 2026-09-09**: migration 0273, `locations_resolved`, the depth function 07 will share, and the paths, parent picker and rolled-up filters in the UI. The product half is unchanged — taxonomy means an additive parent-group change after 03; packaging means 07 after 08 has been used. Resolve Q6 before scheduling the product work. **[28](28-open-container-measurement.md) joined the wave 2026-09-09** and does not wait on Q6: it edits `stock_current` below the product, where 07 edits it above, and running them apart means rewriting that aggregation twice. **[29](29-working-container-replenishment.md) joined the same day** and waits on nothing but ADR-0022's entry-scoped tare — most of what it needs is already shipped. **Q6's sampling was done 2026-09-13 and landed on taxonomy**, proposed as [ADR-0023](../adr/0023-taxonomy-is-groups-packaging-is-parent-product.md). **That record is Proposed, so the product half of this wave is not yet settled**: [07](07-nested-products.md) stays blocked and [30](30-nested-product-groups.md) and [31](31-directed-substitution.md) are drafts rather than its replacements. Two pull requests stand between here and that, deliberately separate: one accepting the record with bookkeeping only, and a later one retiring 07 and scheduling these two. Once that is done 30 is small — [08](08-nested-locations.md) with the nouns changed — and 31 exists because making the forms separate products removes the only relation the schema can express between them. |
