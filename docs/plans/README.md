...
| 07 | [Nested products](07-nested-products.md) | Blocked on Q6 | Decide taxonomy versus packaging from the real catalogue; 08 precedes packaging hierarchy work and has landed, so Q3's shared depth cap already exists as `hierarchy_depth_limit()`. [28](28-open-container-measurement.md) rewrites the same `stock_current` aggregation and is scheduled beside it. |
