...
- **Migration numbering.** Two files, claiming **0283** (medication master data and subjects)
  and **0284** (regimens, administrations, excursions) — 0275–0276 until 2026-09-14, when the
  ...
  free slot instead, and finally 0283–0284 later the same day when
  [issue 176](https://github.com/datagen24/victual/issues/176)'s follow-up to plan 19 piece 2
  was written as `0282.pgsql.php` and took that slot with a file behind it, with rows added to
  [RESERVATIONS.md](../../migrations/RESERVATIONS.md) before any file is written. 0274 belongs to
  [23](23-storage-classes.md), which lands first. **These numbers have moved twelve times** — claimed
  ...
  issue 130, then 0282–0283 for plan 19 piece 2's collision with it, and now 0283–0284 for
  that plan's own written follow-up — so
  ...
  last six are the ones to know about: 0267 went to a defect fix that was already written,
  ...
  it was in flight, and issue 176's fix to that plan's own review follow-up, which was written
  and on disk when this table was next read — so this plan's numbers have six times moved for
  work that was closer to having a file than this one is.
