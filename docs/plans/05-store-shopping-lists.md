...
So B's `(shopping_location_id, product_group_id, sort_number)` table ships as
`NNNN.sqlite.sql` / `NNNN.pgsql.sql`. Since B waits on Q2 and may never happen, it takes
its own number at the time rather than reserving one now — the two engines are allowed to
sit at different migration numbers (see the roadmap's ground rules), so there is nothing to
hold a slot for.

**Verification** is `.devtools/pgsql/difftest.php` on any view these columns reach and
`trigdifftest.php` on the shopping-list triggers, per the standard. Adding a nullable column
changes `SELECT *` output on every view built over these tables, which is the point of
running the diff rather than reasoning about it.
