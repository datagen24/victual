...
The plan corpus is unusually rigorous for a one-household fork. Every plan carries:

- a numbered open-questions section with inline review answers,
- a schema section that names the migration shape,
- an API section that says whether a response changes, and
- a verification section that demands a booted instance rather than lint.

Several plans correct their own earlier reasoning in place and say why (14-Q3 on the CI
image, 14-Q4 on the exemption mechanism, 07-Q6 on what `parent_product_id` means). The
ground rules (additive API, dual-engine migrations from 0256, verification on a real
database) are stated once and enforced by tooling — `check-migrations.php`, the
four-phase differential suite, the rollback tests in CI. That is the strong half.
...
Every row below was re-checked against the tree on 2026-08-30, at `4fa97e8`. **The table
has gone on being a tracker since**: a row whose finding is closed later is updated in
place and carries the date it was closed. The heading above names when this snapshot was
first taken, not the last time any row moved.

"Closed" means the finding was verified gone, not that a commit claimed it — a distinction
this document has more reason than most to insist on, since two of its rows are exactly
the case where a commit claimed it and it was not so. Rows marked "closed 2026-08-30" were
closed by the same branch that added this table; rows marked "closed" without a date were
already fixed before it; a row carrying any other date was closed after it, by the work
that date names.
...
| B7 | **closed 2026-08-30** | All four corrected against `Grocycode.php`: four entity types with `r` for Recipes, plus the fifth type `l` that 06 then proposed and ADR-0011 rejected on 2026-09-04, an opaque object id (06-Q1's UUID), single colon throughout, and DataMatrix restated as default, not the format. `label-printing.md`'s `grocy:` is now `grcy:`. The file notes why a format that never changes still needs its documentation checked. |
...
| B9 | **closed 2026-08-30** | The three reversed sections are struck, annotated with what the sidecar does instead; the surviving reasoning (auth seam, permissions, tool shortlist) stands, since the spec reads as over-engineered without the integrated version for comparison. The API section's "purely additive routes" becomes the truer claim: no new routes, and what 02 needs from this repo is 14 piece 2. |
...
| D5 | **partly adopted, now a stated rule** | The security sweep adopted it in its preamble; the plans had not. 15-C2's `StockReportsController.php:71,90,107` typifies what rots first. The roadmap's ground rules now cover new citations. Existing bare line numbers stay deliberately unswept — rewriting a hundred references by hand is likeliest to introduce a wrong one. A stale line number is a weaker failure than a confidently wrong symbol. |
...
first plan to publish an image from the Dockerfile, so sweep S25 ... is 10's"), while the
sweep assigns it to 15. Plan 10 does not mention `.dockerignore` either. So an item
...
false one.* The difference is that S4's inconsistency was corrected. These two were not,
because nothing re-reads a routing sentence once it is written. That is the argument
...
that can ride with any PR that opens the file. Hardening a production image cannot: it is
meaningless before the image exists and mandatory in the same commit as it. The sweep now
points at 10 instead of claiming it.
...
here are not a work log — they are a claim about the tree, and the tree moves.

Re-verify before trusting a row, and prefer the plans: an item routed to 15-C11 or 10 lives
in a plan that gets read when the work is scheduled, which is more than this document can
say for itself.
...
| # | Finding | Where | Consequence |
|---|---|---|---|
| B1 | **16's breaking renames rest on "no client exists"; 17 names two.** `GROCY-API-KEY` → `VICTUAL-API-KEY` and `grocy_version` → `victual_version` are recorded in 16's Executed section, justified "since no client exists". 17 measures Grocy-SwiftUI at 47 of 48 endpoints working, the Home Assistant integration fully covered, both sending `GROCY-API-KEY`, the iOS app reading `grocy_version.Version`. 17 states "both clients work against the fork today" — false after 16 merged. | `docs/plans/16:290–296`, `docs/plans/17:20–22,61,88–96`, `app.php:96` | Requests from either stock client answer 401. 17's coupling 2 analysis ("one warning banner") assumed the key still existed; a missing key is a Swift decode question, not a banner. 17 needs a reconciliation section against 16's Executed section; 16's justification needs correcting. Cheapest optional mitigation: alias `GROCY-API-KEY` and emit both version keys until 17 decides per client. |
...
| B3 | **Wave 4 schedules 07; 07-Q6 says it may collapse into 03**: "settle this before any of 07 starts — it decides whether 07 is the largest item on the roadmap or one of the smallest". Likely answer: nested `product_groups` (03's territory), `parent_product_id` left at depth one. The README's Wave 4 text, 07's "large" sizing, and 03's scope went unrevisited. | `docs/plans/07:177–202`, `docs/plans/README.md:184–190`, `docs/plans/03` | The largest item on the roadmap has an unresolved scope gate that the roadmap does not show. Add the gate to Wave 4 explicitly, and give 03 a note that it may inherit a `parent_product_group_id` column. |
...
|---|---|---|
| D1 | **Verification sections specify what to run; only plan 16 records what was run.** 13's seven checks evidently ran (the rollback tests exist, the suite passes) but the plan carries no results, dates, or engine versions. The PR template asks for "what was actually run", so evidence exists in PR bodies, but the plan is where the next person looks. | Adopt 16's Executed/Verification pattern as mandatory at close-out: one section per landed plan, results not intentions. |
...
against the protocol's published text. Every downstream design choice in that spec rests
on that description, and it carries no citation, so that check is worth doing before the
sidecar is built. It did not read the two external clients' source; 17's
