...
| [0019](0019-label-printers-are-master-data.md) | Label printers are master data; a separate worker pulls print jobs over an authenticated API | **Proposed**, supplies the configuration, transport and ownership split [0011](0011-label-namespace.md) item 4 left open; relies on 0010; items 1-5 reconciled against [0021](0021-label-templates-are-application-data.md) 2026-09-07, **which must be accepted first**; **all five gates met**; the two format-dependent details written into items 4 and 5 | [22](../plans/22-medication-tracking.md) Q6 (unowned by it), [20](../plans/20-container-infrastructure.md) piece 5 |
...
| [0021](0021-label-templates-are-application-data.md) | Label templates are application data, a reprint is a new job over retained bytes, and an import refuses live labels | **Proposed**, **supersedes three boundaries of [0011](0011-label-namespace.md)**; **all six acceptance prerequisites met** 2026-09-07; **accepted before [0019](0019-label-printers-are-master-data.md)** | [27](../plans/27-label-templates-and-rendering.md), [25](../plans/25-label-infrastructure.md) |
...
| 0019 | Proposed. **All five acceptance gates met**: the worker image built by Nix and asserted shell-free against the image's own contents with a negative control that fires; claim, lease, fence and crash-after-send semantics run against PostgreSQL; rotation, including the integrated claim, send, rotate and report case; the settings schema subset, with rejected writes shown to leave storage unchanged; and the capability contract exercised against the two device families this deployment has, which is where `artifact_forms`, `completion_evidence` and `provenance` came from. Its items 1-5 are reconciled against [0021](0021-label-templates-are-application-data.md) and the two format-dependent details are written. **0021 is accepted first** — 0019's ownership model is the one 0021 decides, and 0011's contrary assignment still stands until then. |
