...
| [0019](0019-label-printers-are-master-data.md) | Label printers are master data; a separate worker pulls print jobs over an authenticated API | **Proposed**, supplies the configuration, transport and ownership split [0011](0011-label-namespace.md) item 4 left open; relies on 0010; items 1-5 reconciled against [0021](0021-label-templates-are-application-data.md) 2026-09-07, **which must be accepted first**; two format-dependent details outstanding before acceptance | [22](../plans/22-medication-tracking.md) Q6 (unowned by it), [20](../plans/20-container-infrastructure.md) piece 5 |
