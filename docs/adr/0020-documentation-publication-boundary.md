...
`config-dist.php`. All of it is read on GitHub. [Plan 26](../plans/landed/26-documentation-site.md)
...
and, since [pull request 92](https://github.com/datagen24/victual/pull/92) landed on
2026-09-07, `docs/data-model.md` and six generated ORM and ERD diagrams. The PHP API carries
PHPDoc throughout and `phpdoc.dist.xml` turns it into a browsable reference, but that
reference is a local build artifact — `.phpdoc/` is gitignored — so it exists only for
...
document written afterwards, which is what makes it an ADR rather than a step in plan 26.
...
building per tag. [Plan 26](../plans/landed/26-documentation-site.md) owns the build, the
...
below. It does not name the phpDocumentor version the two runtimes pin to; that belongs to
plan 26, which has to move both pins together.
...
plan 26's build and not this record.
...
and nobody would notice from the output alone. Plan 26 carries a check that compares them,
...
1. **Met 2026-09-07.** [Pull request 92](https://github.com/datagen24/victual/pull/92) landed,
   ...
   `master`.
2. **Met 2026-09-14.** [Plan 26](../plans/landed/26-documentation-site.md) records how the
