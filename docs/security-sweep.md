...
**Update, 2026-09-17:** [14](plans/14-contract-and-regression-scaffolding.md) piece 2
...
| S10 | **Med** — *fixed* | **Unbounded upload size and unbounded downscale cache.** `UploadFile` streams the raw body with no cap (a raw PUT is not subject to `post_max_size`). `FilesService::GetFilePath` names cache files from unclamped `best_fit_width`/`best_fit_height` (only `is_numeric`), so every distinct pair decodes and resizes the image again and writes a new file. Disk/CPU DoS for any account. | `controllers/Api/FilesApiController.php::UploadFile`, `services/FilesService.php::GetFilePath` | Cap upload size (413 above ~20 MB); clamp best-fit to a small allow-list of sizes. **Done 2026-09-02 with [01](plans/01-file-storage.md)** (`520919d`): `FILE_STORAGE_MAX_SIZE_MB`, default 64, enforced *while* the body is streamed rather than after it is stored — so it bounds the write, which is what the deferred half of S2's fix was about — with both storage backends discarding what they had written and the API answering 413 with the limit named. The effective limit is clamped to the smallest of the setting, `upload_max_filesize` and `post_max_size`, logged once by the CLI boot (`bin/victual-migrate`; from 2026-09-19, [issue 217](https://github.com/datagen24/victual/issues/217) — the earlier per-process memo did not survive a php-fpm request, so web workers logged it on every request) and reported by `GET /api/system/config`, so a configured value PHP will not honour is visible rather than a silent lie. `best_fit_height`/`best_fit_width` snap to the nearest of 32, 64, 250, 400 and 800 — every size the front end asks for, plus one larger default — rather than 400ing, so existing clients keep working while the number of distinct decodes and cached copies is bounded. |
| S11 | **Med** — *fixed* | **API key accepted from the query string; keys never expire.** `ApiKeyAuthMiddleware::AuthenticateRequest` falls back to `?VICTUAL-API-KEY=` — it lands in access logs, browser history and `Referer`. `ApiKeyService::CreateApiKey` sets expiry to 2999. | `middleware/Auth/ApiKeyAuthMiddleware.php`, `services/ApiKeyService.php::CreateApiKey` | Drop the query-string path (the iCal `secret` is the only legitimate URL-borne key and has its own branch); add expiry/rotation to keys. Plan 02's bearer-key seam should not inherit the query path. **The query-string path is gone, 2026-09-04 with 15-C1** — `ApiKeyAuthenticator` reads the header and, on the calendar route only, the `secret` parameter, which is what that affordance exists for. A key in `?VICTUAL-API-KEY=` is a 401. **Keys are also no longer stored in plaintext**, 2026-09-04 with plan 11's question 4 — migrations 0263 and 0264 replace each regular key with its SHA-256 hash and keep the last four characters as a hint, so a database dump is no longer a set of live credentials. A special-purpose calendar key is deliberately left readable and the reasoning is recorded in 0264 and in plan 11. **Expiry and rotation landed 2026-09-15** ([issue 130](https://github.com/datagen24/victual/issues/130), plan 11's own listed follow-up), closing the finding outright. `ApiKeyService::CreateApiKey()` gives a regular key a real, finite `expires` — the caller's chosen lifetime in days, clamped to the new `VICTUAL_API_KEY_MAX_LIFETIME_DAYS` (default 365), or that maximum when none is given — instead of the year-2999 default; every other key type (calendar, and the label worker/verifier/renderer credentials `LabelWorkerCredentialService` writes directly and which never call this method) is untouched, so ADR-0019's paired rotation and the calendar sharing key keep their own stories. Rotation is create-a-successor-then-retire: `ApiKeyService::RotateApiKey()` creates a new key of the same type and description, recorded against migration `0280`'s `api_keys.rotated_from_id`, and leaves the predecessor exactly as it was — it keeps authenticating with no gap, and retiring it is the caller's own separate act (the existing ownership-checked `DELETE /api/objects/api_keys/{id}`), never a side effect of rotating. The manage-keys screen gained an expiry field on creation and a "Rotate" action per regular-type row; both are POST, for the same CSRF reason `/manageapikeys/new` already is (S8). All state is a real column on `api_keys` — no process-memory or APCu state, per [ADR-0007](adr/0007-auth-state-outlives-the-process.md). **Review before merge found a real defect and it is fixed in the same change**: `RotateApiKey()` minted the successor owned by whoever clicked Rotate rather than by the predecessor's own user, so an admin rotating a household member's key handed that member's replacement to the admin's own account — `CreateApiKey()` now takes an explicit owner, defaulting to the current user for every caller except `RotateApiKey()`, which passes the predecessor's. See plan 11's landed record for the detail. Verified against real PostgreSQL 16.13 with a new `.devtools/pgsql/apikey-tests.php` suite phase (`run-tests.sh apikeys`, 31/31 assertions, and `run-tests.sh all` green alongside it — 2026-09-15): a key with a short lifetime is accepted before its stored expiry and refused after; a rotated key's predecessor keeps authenticating through the rotation and is refused only once explicitly deleted, while its successor authenticates throughout and is unaffected by the predecessor's deletion (`ON DELETE SET NULL`, not `CASCADE`); an over-long requested lifetime is clamped rather than refused; a calendar or label-worker-type key created through `CreateApiKey()` still gets the year-2999 expiry; rotation is refused for a non-default key type both in the service and in `OpenApiController::RotateApiKey()`; the hint/hash behaviour from 0263/0264 is unchanged for a rotated key; and, driving the controller itself (not just the service) through a real Blade render, a non-admin rotating someone else's key gets the same 404 a missing row would (so ids cannot be enumerated), a non-integer id is refused the same way, an admin may rotate a key that is not theirs, and both `CreateNewApiKey` and `RotateApiKey` render the new plaintext once rather than redirecting to it. **Not run**: the manage-keys page in an actual browser, and the `frontend-security` job. This sandbox's PHP is 8.4.19; the standalone `.devtools/pgsql/` scripts (this suite included) never load `app.php` and so are unaffected, but the application's own `PrerequisiteChecker` refuses every HTTP route below the PHP 8.5.0 `composer.json` requires, which this sandbox does not have installed. The new Rotate confirmation dialog was added to `.devtools/frontend/s29-payload.js` (`manageapikeys-rotate`) as a second bootbox sink on the same page for when that job next runs, but it was not run here for the same reason. |
| S12 | **Med** — *fixed* | **No brute-force protection on login; default `admin`/`admin`.** `DefaultAuthMiddleware::ProcessLogin` runs `password_verify` on every attempt with no counter or delay; `migrations/0027.php` seeds `admin`/`admin` with no forced change. | `middleware/Auth/DefaultAuthMiddleware.php`, `migrations/0027.php` | Per-IP/per-user throttle; force a password change while the seeded hash is in use. **The throttle state has to outlive the pod.** On the scale-to-zero target a counter held in process memory (or APCu) is reset every time the pod scales down, so an attacker resets it by waiting out the idle window — and per [17](plans/17-ecosystem-clients.md)'s Q2 those windows are long and ordinary. Put it in Redis, which is always-on in the cluster, or in a table. **Done 2026-09-04 in wave 2, in a table** (`login_attempts`, migration 0262, a per-engine pair) — so the counter survives the pod sleeping, which is the whole of why this note is here. One counter, **per username**, in a rolling window; `LOGIN_THROTTLE_MAX_ATTEMPTS` (10) and `LOGIN_THROTTLE_WINDOW_MINUTES` (15) configure it, and 0 turns it off. Hitting the limit is answered exactly like a wrong password, because saying so tells a guesser where the limit is. Rows are written on failure, dropped on the next success for that identity, and pruned to the window on every write, so a healthy installation's table is empty. The forced change is a stored flag rather than a check, because checking means an Argon2id verification on every request: the login path is the only place a plaintext password exists, so that is where the question is answered. It is the **`users.must_change_password` column** (migration 0265) and not the user setting the first draft used — review of this pull request pointed out that a setting is a bag its owner can empty, and `DELETE /api/user/settings/must_change_password` lifted the restriction without changing any password. While the flag is set, every *rendered page* redirects to the account's own edit form with the password fields open; API routes are deliberately untouched, since that form saves through the API. **Revisited 2026-09-19** after CodeRabbit's review of PR #211 pointed out what "untouched" means on a fresh deployment: whoever logs in with admin/admin before the operator does holds the whole API — `POST /api/users` included, a second administrator that outlives the password change the pages then force. Two changes close it. A fresh database no longer gets a known password at all: `InitialDataSeeder` takes `VICTUAL_BOOTSTRAP_ADMIN_PASSWORD` from the migrate container's environment, or generates one, prints it once to the migration's stderr and flags it (login now only ever *raises* the flag, so using the printed password does not lift it). And a flagged account gets `403` on every API route except the three the change-password form needs (`PUT /api/users/{own id}`, `GET /api/user`, `GET /api/system/db-changed-time`), by session and by API key alike. What remains: an installation seeded before this that is *still* on admin/admin is claimable by whoever logs in first, as before — but the claimant is now limited to changing the password, which locks the operator out visibly instead of leaving them working beside a planted account. `tests/Pgsql/BootstrapAdminTest.php` holds both halves.
...
- **The label-printer webhook is gone.** Plan [32](plans/32-label-kinds.md) deleted
