...
class PrintEvidenceService extends LabelService
{
    public function Submit(int $workerId, int $attemptId, array $evidence): array
    {
        ...
        $submittedConfidence = isset($evidence['confidence']) ? (float)$evidence['confidence'] : null;
        if ((int)$row['attempt_id'] !== $attemptId || $row['evidence_type'] !== $evidence['evidence_type'] || json_decode($row['detail'], true) != $evidence['detail'] || $row['decoded_uid'] !== ($evidence['decoded_uid'] ?? null) || json_decode($row['printer_status'] ?? 'null', true) != ($evidence['printer_status'] ?? null) || (new \DateTimeImmutable($row['observed_at'])) != (new \DateTimeImmutable($evidence['observed_at'])) || ($row['confidence'] === null ? null : (float)$row['confidence']) !== $submittedConfidence) {
