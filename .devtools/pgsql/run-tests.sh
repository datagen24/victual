...
#
# The fifteenth is above the freeze like the thirteenth and fourteenth - its subject is
# migrations/0273.pgsql.sql - and it is here rather than in the view or trigger phases because
# neither could see what it asks. The view phase seeds SQLite and copies the tables across
# through the importer's common-column logic, so locations.parent_location_id would arrive
# NULL for every row and every assertion about a tree would be an assertion about a flat list.
# The trigger phase applies the same script to both engines and diffs the tables afterwards,
# which cannot express "this write was refused, and refused for this reason" - and three
# different rules refuse a write to locations, so a phase that only asked whether one was
# refused would pass when the wrong one fired. It also runs past the database: the delete
# refusal has to arrive at a client as a sentence rather than as a driver message, since
...
#
# The seventeenth is above the freeze like the thirteenth through sixteenth - its subject is
...
#
# The eighteenth is above the freeze like the thirteenth through seventeenth - its subject is
# migrations/0278.pgsql.sql - and it is here for the fifteenth's own reason, one table over:
# the view phase seeds SQLite and copies the tables across through the importer's
# common-column logic, so product_groups.parent_product_group_id would arrive NULL for every
# row and every assertion about a tree would be an assertion about a flat list, and the
# trigger phase cannot express "this write was refused, and refused for this reason" when two
# different rules refuse a write to the same table. Its cases follow plan 30's own Verification
# list, including the mixed node ADR-0023 decision 6 claims needs no special case - a group
...
# --- Shopping list store link tests -------------------------------------------------
#
# Plan 05 parts A and C (issue #85), migrations/0286.pgsql.sql. Written directly in tier 1
# (ADR-0025 decision 7 - new code arrives with its own tests) rather than as a 25th bespoke
# script: same shape as run_contract_tests()/run_rbac_tests(), an empty database
# PgsqlSchemaTestCase migrates itself, per class, into its own schema.

run_shopliststores_tests() {
	local dbname="victual_shopliststores"
	dropdb --if-exists "$dbname" || fail "could not drop $dbname"
	createdb "$dbname" || fail "could not create $dbname"

	local datapath="$SUITE_SCRATCH/shopliststores-data"
	rm -rf "$datapath"
	mkdir -p "$datapath"
	mkdir -p "$datapath/viewcache"
	cat > "$datapath/config.php" <<-PHPCONFIG
	...
	say ""
	if ! VICTUAL_DATAPATH="$datapath" PHPUNIT_DB_NAME="$dbname" \
		php "$VICTUAL_ROOT/packages/bin/phpunit" --configuration "$VICTUAL_ROOT/phpunit.xml" --testsuite shopliststores; then
		failures=$((failures + 1))
	fi
...
# --- Credential split ---------------------------------------------------------------
#
# Plan 20 verification 8, issue #133: deploy/postgres/roles.sql gives the serving image a
# database role with no DDL rights and the migrate image the only one that can migrate, and
# the application still connects and works on the restricted one. Tier 1 like rbac, but the
# test makes a database of its own rather than a schema inside this one - the script grants
# on `public` - so this phase only has to supply the empty database the test connects to
# for CREATE DATABASE, and a data directory for the bootstrap. It needs a role that can
# create roles, which the suite's PGUSER is everywhere the suite runs.
run_credentialsplit_tests() {
	local dbname="victual_credentialsplit"
	dropdb --if-exists "$dbname" || fail "could not drop $dbname"
	createdb "$dbname" || fail "could not create $dbname"

	local datapath="$SUITE_SCRATCH/credentialsplit-data"
	rm -rf "$datapath"
	mkdir -p "$datapath"
	mkdir -p "$datapath/viewcache"
	cat > "$datapath/config.php" <<-PHPCONFIG
	...
	say ""
	if ! VICTUAL_DATAPATH="$datapath" PHPUNIT_DB_NAME="$dbname" \
		php "$VICTUAL_ROOT/packages/bin/phpunit" --configuration "$VICTUAL_ROOT/phpunit.xml" --testsuite credentialsplit; then
		failures=$((failures + 1))
	fi
...
# GET /mealplan rendered for a role that may and a role that may not see the price-bearing
# recipes_resolved fields (plan 19 piece 2's redaction, issue #176 item 4). Tier 1 like
# shopliststores: an empty database PgsqlSchemaTestCase migrates itself into its own schema.

run_mealplan_tests() {
	local dbname="victual_mealplan"
	dropdb --if-exists "$dbname" || fail "could not drop $dbname"
	createdb "$dbname" || fail "could not create $dbname"

	local datapath="$SUITE_SCRATCH/mealplan-data"
	rm -rf "$datapath"
	mkdir -p "$datapath"
	mkdir -p "$datapath/viewcache"
	cat > "$datapath/config.php" <<-PHPCONFIG
...
# --- MCP key type, read-only keys and the capabilities endpoint ----------------------
#
# Issue #208, through the whole middleware stack in production mode
# (tests/Pgsql/McpAuthTest.php): MCP keys in the header, the read-only 403, the
# VICTUAL-API-KEY-TYPE narrowing and GET /api/user/capabilities. Same shape as
# run_rootentry_tests(), whose subprocess helper tests/Pgsql/request-subprocess-helper.php
# generalises to any request.
run_mcpauth_tests() {
	local dbname="victual_mcpauth"
	dropdb --if-exists "$dbname" || fail "could not drop $dbname"
	createdb "$dbname" || fail "could not create $dbname"

	local datapath="$SUITE_SCRATCH/mcpauth-data"
	rm -rf "$datapath"
	mkdir -p "$datapath"
	mkdir -p "$datapath/viewcache"
	cat > "$datapath/config.php" <<-PHPCONFIG
...
# --- Upload clamp announcement ---------------------------------------------------
#
# Issue #217. FileSizeLimit kept the effective upload limit in a static property and
# logged the clamp when it first filled it, calling that "once per process" - but a static
# property does not outlive a php-fpm request (ADR-0007), so the line went out on every
# request. ConfigurationValidator now announces it only under the CLI SAPI, which every
# deployment shape runs exactly once before serving (bin/victual-migrate). The test boots
# the configuration and the validator under php and under php-cgi with different php.ini
# values and reads what reached error_log. No database.
...
# --- Nested locations tests --------------------------------------------------------
#
# PostgreSQL only: migrations/0273.pgsql.sql is above the SQLite freeze, so there is no second
# engine holding the column, the view or the three guards. A migrated database and nothing
# else - the phase builds its own tree, because the base fixture's four locations are flat and
# the file it lives in is loaded by the SQLite side too.

run_nested_locations_tests() {
	local dbname="victual_nested_locations"
	build_pgsql "$dbname"

	local datapath="$SUITE_SCRATCH/nested-locations-data"
	rm -rf "$datapath"
	write_pgsql_config "$datapath"

	# The same reason the group minimum phase makes this directory: the phase writes through
	# the API, so HTMLPurifier runs and wants somewhere to serialise its definition cache.
	mkdir -p "$datapath/viewcache"
...
# --- Nested product groups tests ----------------------------------------------------
#
# PostgreSQL only: migrations/0278.pgsql.sql is above the SQLite freeze, so there is no second
# engine holding the column, the view or the two guards. A migrated database and nothing else -
# the phase builds its own tree, because product_groups starts empty after a fresh migrate
# (InitialDataSeeder seeds quantity units and locations, not product groups).

run_nested_product_groups_tests() {
	local dbname="victual_nested_product_groups"
	build_pgsql "$dbname"

	local datapath="$SUITE_SCRATCH/nested-product-groups-data"
	rm -rf "$datapath"
	write_pgsql_config "$datapath"

	# The same reason the nested locations phase makes this directory: the phase writes
	# through the API, so HTMLPurifier runs and wants somewhere to serialise its definition
	# cache.
	mkdir -p "$datapath/viewcache"

	say ""
	if ! VICTUAL_DATAPATH="$datapath" DIFFTEST_DB_NAME="$dbname" php "$SUITE_DIR/nested-product-groups-tests.php"; then
		failures=$((failures + 1))
	fi
...
# --- Directed product substitution tests --------------------------------------------
#
# PostgreSQL only, for the reason the nested product groups phase gives: migrations/0279.pgsql.sql
# is above the SQLite freeze, so product_substitutions and product_substitutions_resolved have
# no second engine to compare against. The view phase's own products_current_substitutions case
# already covers the existing dual-engine mechanism this migration deliberately leaves alone.

run_product_substitutions_tests() {
	local dbname="victual_product_substitutions"
	build_pgsql "$dbname"

	local datapath="$SUITE_SCRATCH/product-substitutions-data"
	rm -rf "$datapath"
	write_pgsql_config "$datapath"

	# The phase writes through GenericEntityApiController, so HTMLPurifier runs and wants
	# somewhere to serialise its definition cache, the same reason the nested product groups
	# phase makes this directory.
	mkdir -p "$datapath/viewcache"

	say ""
	if ! VICTUAL_DATAPATH="$datapath" DIFFTEST_DB_NAME="$dbname" php "$SUITE_DIR/product-substitutions-tests.php"; then
		failures=$((failures + 1))
	fi
...
# read-then-write window between deciding a booking is allowed and writing it. Nothing
# else in the suite drives two live database connections against the same booking at
# once - the differential and stockcoverage phases each ask one connection a question at
# a time - so this is a harness of its own: a second raw PDO takes the same advisory lock
# a booking would, a subprocess drives the booking under test through the real HTTP stack
# so it blocks on that lock, the phase polls pg_stat_activity until it is actually
# waiting, and only then does the second connection commit its competing write.
#
# Same shape as run_stockcoverage_tests(): an empty database PgsqlSchemaTestCase migrates
# itself, per class, into its own schema.

run_stockconcurrency_tests() {
	local dbname="victual_stockconcurrency"
	dropdb --if-exists "$dbname" || fail "could not drop $dbname"
	createdb "$dbname" || fail "could not create $dbname"

	local datapath="$SUITE_SCRATCH/stockconcurrency-data"
	rm -rf "$datapath"
	mkdir -p "$datapath"
	mkdir -p "$datapath/viewcache"
	cat > "$datapath/config.php" <<-'PHPCONFIG'
	...
	say ""
	if ! VICTUAL_DATAPATH="$datapath" PHPUNIT_DB_NAME="$dbname" \
		php "$VICTUAL_ROOT/packages/bin/phpunit" --configuration "$VICTUAL_ROOT/phpunit.xml" --testsuite stockconcurrency; then
		failures=$((failures + 1))
	fi
...
case "$WHICH" in
	...
	productgroups) run_nested_product_groups_tests ;;
