// Powers the product substitution create/edit form (views/productsubstitutionform.blade.php), embedded in a modal
// from the product edit form: saves a directed edge via the objects/product_substitutions API.

// Form submit: POSTs objects/product_substitutions (create) or PUTs objects/product_substitutions/{id} (edit, id
// from Victual.EditObjectId). The form only knows "this product" (fixed, hidden) and "the other product" (picked);
// direction says which side of the edge "this product" is on, so it is translated into from_product_id/to_product_id
...
$('#save-product-substitution-button').on('click', function (e)
{
	...
	{
		Victual.Api.Put('objects/product_substitutions/' + Victual.EditObjectId, jsonData,
			function (result)
			{
				window.parent.postMessage(WindowMessageBag("ProductSubstitutionsChanged"), Victual.BaseUrl);
				window.parent.postMessage(WindowMessageBag("CloseLastModal"), Victual.BaseUrl);
