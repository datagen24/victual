...
- Prices are a permission now, not just an instance-wide setting. `STOCK_PRICES_VIEW` sits under `STOCK_PURCHASE`, so anyone who records purchases still sees what things cost and nothing changes for an account holding `STOCK` or `ADMIN`. The seeded Child and Guest roles hold neither, so they no longer see prices anywhere - and neither does an account whose only stock grant is `STOCK_VIEW`, which could read every price before
- A field you may not see is now *absent* from an API response rather than `null`. That distinction is deliberate: `stock_log.price` is legitimately `null` for a consumption, and "there was no price" has to stay tellable from "you may not see it". The fields are `price` on `stock`, `stock_log` and `stock_next_use`, `value` on `stock_current`, `last_price` on `product_barcodes` and `product_barcodes_view`, `costs`, `costs_per_serving` and `prices_incomplete` on `recipes_resolved`, `costs` on `recipes_pos_resolved`, `price` on `products_average_price` and `products_last_purchased`, the three price columns of `uihelper_stock_current_overview`, the three of `uihelper_shopping_list`, and `last_price`, `avg_price`, `oldest_price`, `current_price` and `stock_value` on `GET /stock/products/{productId}`
- Naming a field you may not see in `query[]` or `order` is answered `400` rather than applied, so a price cannot be found by filtering or sorting on it either. The message distinguishes "no such field" from "you may not use this one"; both are `400`, since a distinct status would itself confirm the field exists
- `GET /stock/products/{productId}/price-history` and the `/stockreports/spendings` page answer `403` without `STOCK_PRICES_VIEW`. Both are entirely prices, so they refuse rather than return an empty shape
- The pages that show prices - stock overview, stock entries, the shopping list, recipes, the meal plan, the product card and the product form's barcode table - omit them for a caller who may not see them, rather than hiding them with CSS. There is nothing in the page source to read
