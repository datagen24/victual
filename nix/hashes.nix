...
# See nix/README.md, "Bootstrapping the hashes".
{
  ...
  #
  # Still the fakeHash placeholder because that lockfile does not exist yet — it is
  # generated by `npm install` in an environment with npm registry access, which the
  # sandbox that scaffolded nix/mcp.nix (issue #86) did not have. The local session
  # that installs the MCP TypeScript SDK for the first time gets the real value the
  # same way `nix build .#app` and `.#frontend` give the two hashes above: the first
  # `nix build .#mcp` fails on purpose and names it. This file takes no `{ lib }:`
  # argument, so the literal is spelled out rather than written as `lib.fakeHash`.
  mcpNpmDeps = "sha256-AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=";
}
