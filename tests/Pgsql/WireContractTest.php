 ...
 *        row - are still integers.
 *   #231 No field is typed `format: date-time` unless it really is RFC 3339, and the one
 *        that is stays.
 *   #232 The GET /objects/{entity} union members are mutually exclusive, so no entity's
 *        rows decode as another entity's schema.
...
class WireContractTest extends PgsqlSchemaTestCase
{
	...
	/**
	 * Every property the document types `boolean` is either produced by a site that already
	 * converts it or named in WireBooleans::COLUMNS. Adding a boolean property to the
	 * document without wiring it is the omission issue #230 was, and this is what makes the
	 * next one fail here rather than in a client.
	 */
	public function testEveryDocumentedBooleanIsAccountedFor(): void
	{
		// Computed in PHP rather than read from a column, and boolean before this existed.
		$alreadyBooleanInPhp = ['has_childs', 'read_only'];

		$documented = [];
		self::collectBooleanProperties(self::spec()['components']['schemas'], $documented);

		$covered = $alreadyBooleanInPhp;
		foreach (WireBooleans::COLUMNS as $columns)
		{
			$covered = array_merge($covered, $columns);
		}

		$unaccounted = array_values(array_diff(array_unique($documented), array_unique($covered)));
		sort($unaccounted);

		self::assertSame(
			[],
			$unaccounted,
			'documented boolean properties with nothing converting them: ' . implode(', ', $unaccounted)
		);
	}

	/**
	 * The names of every property any schema below $node types `boolean`.
	 *
	 ...
	 * property this class is responsible for.
	 *
	 * @param array<string,mixed> $node
	 * @param string[] $into
	 */
	private static function collectBooleanProperties(array $node, array &$into): void
	{
		...
		foreach ($node as $child)
		{
			if (is_array($child))
			{
				self::collectBooleanProperties($child, $into);
			}
		}
	}
